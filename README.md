# TRAVEO&trade; T2G: P-DMA 1D transfer 

This code example demonstrates the transmission of data from memory to memory via peripheral direct memory access (P-DMA) 1D transfer, including its operation, initial settings, and interrupt handling.


[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243222-pdma-mem-to-mem)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce243222-pdma-1d-transfer](https://github.com/Infineon/mtb-example-ce243222-pdma-mem-to-mem)" application with the desired name "P-DMA_1d-transfer" configured for the *KIT_T2G-B-H_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G-B-H_LITE --app-id mtb-example-ce243222-pdma-1d-transfer --user-app-name P-DMA_1d-transfer --target-dir "C:/mtb_projects"
   ```
...
The application demonstrates a P-DMA 1D transfer, which is a memory to memory transfer between two buffers in SRAM as P-DMA can be also used for transfers between memories, but the transfer bandwidth may not be enough when compared with M-DMA.
This design consists of P-DMA and USER_BTN1. P-DMA is set to initiate transfers by software triggers.The source buffer and destination buffer contains 100 bytes of data. Pressing USER_BTN1 generates an interrupt. 
When an interrupt occurs, P-DMA transfer is triggered by software. Then, P-DMA transfers data from a source buffer in SRAM to a destination buffer in SRAM.
...
To initiate a P-DMA 1D transfer, a GPIO interrupt from USER_BTN1 is used. The interrupt is triggered on the falling edge of the button signal.
...
P-DMA is configured to perform a 1D transfer on a single trigger. The X-loop count is set to 100 and the Y-loop count is set to 1. The P-DMA is configured in Byte-to-Byte mode with source increment of 1 byte and a destination increment of 1 byte, resulting in transfer of the data from the 100-byte source buffer to the 100-byte destination buffer.
...
Document title: *CE243222  - TRAVEO™ T2G: P-DMA 1D transfer* 
